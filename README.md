...
**InkOS Cockpit 现在也是一级入口。** 运行 `inkos cockpit [bookId]` 可直接打开对话优先的创作驾驶舱。
它现在会打开独立的 `/cockpit/` 外壳；旧的 `/?page=cockpit` Studio 路由会自动转发到 `/cockpit/`，继续作为迁移兼容入口。

**Korean is now a first-class writing language.** Global default is `ko`, and `--lang` can switch to `zh`/`en`.  
현재 지원 언어: `ko`, `zh`, `en`.  
지원 플랫폼: `naver-series`, `kakao-page`, `munpia`, `novelpia`.  
한국 내장 장르: `modern-fantasy`, `fantasy`, `murim`, `romance-fantasy`, `korean-other`.  
영문 전용 설명은 더 이상 기본값이 아닙니다. 자세한 내용은 [English README](README.en.md) 참고.
...
# provider: openai / anthropic / custom / gemini-cli / codex-cli
# gemini-cli：使用本地 Gemini CLI 的 OAuth 登录，不需要 --base-url 和 --api-key
# codex-cli：使用本地 Codex CLI 的 OAuth 登录，不需要 --base-url 和 --api-key
# base-url: 你的 API 提供商地址
# api-key: 你的 API Key
# model: 你的模型名称
# --lang: 项目默认写作语言（ko / zh / en）
```
...
INKOS_LLM_BASE_URL=                               # API 地址（支持中转站、智谱、Gemini 等；CLI OAuth 可留空）
INKOS_LLM_API_KEY=                                 # API Key
INKOS_LLM_MODEL=                                   # 模型名（例如 auto-gemini-3 / gpt-5.4）

# 可选
# INKOS_DEFAULT_LANGUAGE=                            # 全局默认语言: ko / zh / en
# INKOS_LLM_TEMPERATURE=0.7                       # 温度
# INKOS_LLM_MAX_TOKENS=8192                        # 最大输出 token
# INKOS_LLM_THINKING_BUDGET=0                      # Anthropic 扩展思考预算
# INKOS_GEMINI_CLI_SOURCE_HOME=~                   # 可选：Gemini CLI OAuth 凭据所在目录
```
...
```bash
inkos book create --title "吞天魔帝" --genre modern-fantasy --platform naver-series --lang ko  # 创建新书（한국어 내장 장르 + 플랫폼）
...
- 语言计数默认按题材和语言设置：`zh` 默认为 `zh_chars`，`en` 默认为 `en_words`，`ko` 默认为 공백 제외 `ko_chars`
...
| `inkos book create` | 创建新书（`--genre`、`--platform`、`--lang (ko|zh|en)`、`--chapter-words`、`--target-chapters`、`--brief <file>` 传入创作简报） |
...
| `inkos cockpit [bookId]` | 直接进入对话式创作驾驶舱（`-p` 指定端口，可选深链到某本书） |
