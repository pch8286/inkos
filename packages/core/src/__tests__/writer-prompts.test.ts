...
describe("buildWriterSystemPrompt", () => {
  ...
  it("uses Korean contract branch for Korean mode and avoids non-Korean guidance headers", () => {
    ...
    expect(prompt).toContain("독자가 이미 추론할 수 있는 의도, 성격, 관계 판단을 마지막에 장식적 판정문으로 덧붙이지 않는다");
    expect(prompt).toContain("구체 단서가 충분하면 그 문장은 빼고");
    expect(prompt).not.toContain("국화");
    expect(prompt).not.toContain("향보다 계산");
