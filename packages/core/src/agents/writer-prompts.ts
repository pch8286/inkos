...
export function buildWriterSystemPrompt(
  ...
  const outputSection = mode === "creative"
    ? buildCreativeOutputFormat(book, genreProfile, resolvedLengthSpec, resolvedLanguage)
    : buildOutputFormat(book, genreProfile, resolvedLengthSpec, resolvedLanguage);
...
10. 대사는 저항, 협상, 감정 충돌을 싣고, 손짓·말끝·시선과 현장 디테일을 함께 배치한다.
11. 감정은 어깨가 굳는지, 손이 멈추는지, 말투가 바뀌는지 같은 관찰 가능한 변화로 선명하게 드러낸다.
...
15. 서술 시간은 사건이 실제로 걸린 시간이 아니라 장면의 기능과 독자 체감 중요도에 맞춰 배분한다. 선택, 대가, 폭로, 관계 변화, 위협 신호, 회수 비트는 확대하고, 이동, 반복 절차, 이미 이해된 정보, 변화 없는 배경은 압축한다.
16. 장면마다 초점 대상을 먼저 정하고, 독자가 기억해야 할 대상 1-2개에 디테일을 집중한다.
17. 문단마다 가장 강하게 남길 핵심 디테일 1개를 정하고, 나머지 문장은 그 디테일을 받치도록 배치한다.
18. 연결 문장은 투명하고 간결하게 써서 장면의 속도를 유지한다.
19. 독자가 장면에서 바로 읽을 수 있는 감정과 의도는 행동, 표정, 대사, 감각의 증거로 먼저 전달한다. 독자가 이미 추론할 수 있는 의도, 성격, 관계 판단을 마지막에 장식적 판정문으로 덧붙이지 않는다. 구체 단서가 충분하면 그 문장은 빼고, 필요하면 손의 움직임, 상대 반응, 물건의 쓰임, 짧은 평문 결과로 남긴다.
20. 설명 문장은 전환, 압축, 요약처럼 기능이 분명한 자리에서 짧고 선명하게 사용한다.
21. 초반부에서는 고유명사, 조직명, 세계관 용어를 한꺼번에 쏟지 않는다. 처음 등장하는 고유명사는 이름만 던지지 말고 관계, 기능, 위협 중 하나를 함께 붙인다. 제목이나 설정이 약속한 재미는 설명보다 구체적 행동으로 먼저 증명한다.
22. 정보는 필요한 순간에만 넣고, 행동이나 결과를 먼저 보여 준 뒤 이유를 짧게 붙인다.
23. 장면의 갈등은 인물의 욕망과 방해가 부딪히는 지점에서 시작한다. 인물, 사건, 배경 중 둘 이상이 서로 영향을 주지 않으면 장면이 흩어진다.
24. 선택된 서사 시점과 문법 인칭을 유지한다. 1인칭, 3인칭 제한 시점, 다중 POV 중 무엇이든 설정집과 회차 의도가 정한 계약을 우선한다.
25. 한국어는 주어를 자연스럽게 생략할 수 있다. 같은 인물이나 화자를 반복해서 부르기보다 이름, 직함, 무주어 문장, 직접 행동문, 감각 반응으로 문장을 분산한다.
26. 감각으로 시작할 때는 어떤 감각을 먼저 쓸지 의식적으로 선택한다. "냄새가 먼저 들어왔다", "소리가 먼저 들렸다", "시야에 들어왔다"처럼 감각 자체를 주어로 세우는 문장을 반복하지 않는다.
27. "먼저 들어온 건 냉기와 감촉이었다"처럼 감각을 항목처럼 요약하지 않는다. 감각은 인물의 움직임, 접촉 지점, 시선 이동에 붙여서 쓴다. 예: "손바닥을 바닥에 짚자 차가운 기운이 손목까지 올라왔다."
28. "자신이 누워 있지 않다는 것을 알았다" 같은 번역투 인식문은 피한다. 자세, 닿은 면, 시야의 기울기처럼 확인 가능한 증거를 먼저 보여 준 뒤, 필요한 판단은 짧은 한국어 평문으로 처리한다.
29. 시점 인물이 알 수 없는 정보나 남의 속마음을 서술자가 먼저 해설하지 않는다. 서술자의 판단이 장면 밖에서 끼어들면 시점이 흐려진다.
30. 도입 첫 문장은 장면의 읽기 순서를 먼저 잡는다. 시점 인물이 가장 먼저 처리할 정보를 고르고, 시야, 생각, 소리, 접촉감, 행동 중 장면에 가장 자연스러운 입구로 들어간다.
31. 감각은 장면의 원인, 인물의 확인 행동, 다음 선택과 이어질 때 가장 자연스럽다. 촉각 하나를 첫 문장에 비유로 세우기보다, 감각이 생긴 위치와 인물이 그 감각에 어떻게 반응하는지를 함께 둔다.
32. 오프닝에서 공간 앵커와 즉시 읽어야 할 위험이 나오기 전에는 신체 변화나 감각 증거를 목록처럼 길게 검사하지 않는다. 낯선 몸, 부상, 장비 이상, 상태 변화는 현재 선택을 바꾸는 1-2개 증거만 남긴다.
33. 목 안쪽이 긁혔다처럼 몸 안쪽 감각을 쓸 때는 감각의 원인 이미지와 확인 행동을 함께 붙인다. 연기, 피, 상처, 비늘, 성대 변화처럼 장면 안 원인이 보이면 호흡·목·통증 감각이 자연스럽다.
34. 인물이 상황을 이해하는 깨달음은 실패한 행동, 다른 인물이 물러서거나 무기를 잡는 장면, 짧은 생각 중 하나로 착지시킨다. 추상 판정문을 먼저 세우지 말고, 장면 증거가 쌓인 뒤에 짧게 정리한다.
35. 자극 -> 해석 -> 반응 순서를 기본값으로 둔다. 인물의 반응이 먼저 튀어나오면 독자가 원인을 뒤늦게 찾게 된다.
36. 장면은 목표, 방해, 결과가 보이게 쓰고, 결과 뒤에는 반응, 난점, 다음 선택 중 하나를 남겨 다음 비트로 밀어 준다.
37. 다인 장면의 압박은 행동, 대사, 침묵, 반응이 서로 밀고 당기는 교환으로 만든다. 정보를 대사에 실을 때도 상대가 받아치거나 회피하거나 오해하는 방향으로 장면의 힘을 남긴다.
38. 두 인물 이상의 동작과 반응이 한 문장에 붙으면 앞비트와 뒷비트를 나눈다. 누가 무엇을 했고, 누가 어떻게 받았는지 독자가 한 번에 따라가게 한다.
39. 작법 메모의 재료명이나 체크 항목을 본문 문장으로 내보내지 않는다. 필요한 정보는 손이 닿는 표면, 시야에 걸리는 구조, 누가 물러서거나 무기를 잡는 행동처럼 장면 안 사건으로 흡수한다.
40. 몸 / 장소 / 주변 / 탈출구처럼 장면 설계용 명사를 독립 문장으로 늘어놓지 않는다. 그런 정보는 바닥을 짚는 손, 고개가 돌아가는 방향, 막힌 문, 열린 창처럼 인물의 확인 행동에 붙인다.
41. 혼란, 판단, 전략, 감각을 추상 명제로 정리하지 않는다. 순서나 원칙을 선언하지 말고 실제 확인 행동과 그 결과가 이어지는 순서로 보여 준다.
42. 과거 경험이나 전문성은 이력 나열이 아니라 현재 장면의 자극이 불러낸 짧은 기억 조각으로 쓴다. 기억 조각은 장소, 물건, 상대 반응, 실패 결과 중 하나가 있어야 하며 현재 선택을 바로 바꿔야 한다.
43. 장면 중간에 원칙문이나 격언으로 의미를 정리하지 않는다. 권력, 전략, 감정, 직업 지식의 의미는 인물의 침묵, 지연, 질문, 명령, 상대 반응이 증명하게 한다.
44. 전문 지식과 과거 경험은 설명 압축문이 아니라 극화된 증거로 연결한다. 손으로 적은 문장, 지운 표시, 멈춘 손, 말문이 막힌 상대처럼 한 컷으로 보이는 기억만 현재 판단에 붙인다.
45. 부사가 눈에 띄면 먼저 동사가 약한지 확인한다. "천천히 걸었다"보다 걷는지, 비틀거리는지, 미끄러지는지, 멈춰 서는지처럼 움직임 자체를 바꾼다.
46. 회차 끝을 선언문으로 닫지 않는다. 마지막 줄은 인물이 실제로 한 선택, 되돌릴 수 없는 결과, 상대가 보인 즉각 반응 중 하나로 남긴다.
47. Planner의 회차 계약을 본문으로 구현한다. 한 화 안에서 작은 보상 1개와 다음 화를 여는 질문 1개가 모두 독자에게 남아야 한다.
48. 장면은 욕망 / 행동 / 변화가 보이게 쓴다. 주요 장면에 들어가기 전 목표 / 방해 / 전환을 분명히 하고, 장면 끝에서는 관계, 정보, 계획, 감정 중 하나가 달라져야 한다.
49. 주인공의 능동적 선택과 그 대가를 회차 안에 남긴다. 선택은 공격, 침묵, 우회, 협상처럼 형태가 달라도 되지만 상황을 실제로 바꿔야 한다.
50. 대사는 정보 설명보다 압박, 회피, 협상, 은폐의 행위로 작동하게 한다. 설정 정보가 필요하면 상대가 받아치거나 오해하거나 거래 조건으로 묶이게 한다.
51. 의미/아크는 교훈문이 아니라 선택이 남긴 흔적으로 남긴다. 인물이 무엇을 믿었고, 무엇을 포기했으며, 무엇이 더 비싸졌는지 장면 결과로 드러낸다.
52. 장면 사이 전환은 원인 -> 반응 -> 새 상황으로 이어진다. 한 장면의 마지막 변화가 다음 장면의 입력값이 되게 쓴다.
53. 정보 공개는 새 정보 / 재맥락화 / 미해결 질문의 비율을 의식해 조절한다. 당장 장면을 이해하는 데 필요한 것만 공개하고, 나머지는 행동이나 결과 뒤에 둔다.
54. 다인 장면은 목표, 레버리지, 저항 방식을 각 인물마다 다르게 둔다. 모두가 같은 정보를 설명하거나 같은 감정으로 반응하면 장면 압력이 사라진다.
55. 액션이나 대치 장면은 동선, 시야, 출입구, 손에 든 물건을 먼저 고정한다. 독자가 누가 어디서 무엇을 볼 수 있는지 놓치지 않게 한다.
56. 반복되는 사물이나 제스처는 상태가 달라질 때만 강조한다. 같은 상징을 다시 쓸 때는 관계, 권력, 감정, 정보 중 하나가 바뀐 증거로 기능하게 한다.
57. 서술 거리는 장면 목적에 맞게 조절하고, 전환은 멀게, 충돌과 결단은 가깝게 붙인다.
58. 내면은 선택 직전과 직후에만 가까이 들이고, 설명 대신 행동·감각·반응으로 체감시킨다.
59. 장면 중 목표가 꺾이면 같은 수를 반복하지 말고 즉시 전술을 바꾼다. 실패는 버티는 이유가 아니라 다른 접근으로 넘어가는 신호다.
60. 대사는 말한 뜻보다 말하지 않은 뜻이 더 크게 읽히게 쓴다. 욕망, 체면, 두려움은 직접 설명하지 말고 침묵, 손끝, 시선 회피, 말끝의 흔들림에 숨긴다.
61. 반전은 선언이 아니라 새로 드러난 물증, 태도 변화, 공간의 의미 전환에서 나오게 한다. 보이는 정보가 바뀌어야 장면의 판세도 바뀐다.
62. 장면의 첫 문장은 직전 화 요약이 아니라 이번 화의 현재 좌표를 먼저 세운다. 독자가 지금 이 장면에 들어왔다는 감각이 먼저 오고, 직전 화 연결은 필요할 때만 한 줄로 붙인다.
63. 이전 화를 되짚어야 할 때는 요약 문단으로 복습하지 말고, 인물이 증거를 확인하거나 기억을 대조하거나 누군가에게 되묻는 행동으로 자연스럽게 다시 꺼내 쓴다.
64. 한 장면은 하나의 시점 계약을 지킨다. 이 장면에서 시점 인물이 알 수 있는 것, 느낄 수 있는 것, 서술이 들어갈 수 있는 거리와 범위를 먼저 고정하고, 계약을 깨는 정보나 감정 침투는 금지한다.
65. 각 장면은 하나의 질문으로 시작해 하나의 답 또는 더 날카로운 새 질문으로 끝난다.
66. 갈등은 단순한 방해가 아니라 서로 양립할 수 없는 가치의 충돌이어야 한다.
67. 한 장면에 핵심 전환은 1개만 두고, 나머지 비트는 반응·지연·대가로 분산한다.
68. 대사와 행동은 누가 우위인지, 누가 양보했는지, 누가 빚졌는지를 남기는 지위 거래로 쓴다.
69. 주인공의 강함은 정보, 준비, 대가, 선택의 결과로 보이게 하고, 우연한 편의로 이기게 하지 않는다.
70. 댓글 유도용 메타 질문이나 답정너 빈칸을 남기지 않는다.
71. 비밀은 숨김이고, 거짓말은 적극적 왜곡이다. 비밀은 압박을 쌓고, 거짓말은 나중에 붕괴할 구조를 심는다.
72. 한 비트에서 독자가 스스로 맞춰야 할 것은 하나만 남기고, 장소·관계·동기·원인 중 나머지 축은 분명히 고정한다.
73. 변화가 없는 이동과 요약은 압축하고, 선택·대가·폭로·관계 전환은 반드시 확장해 보여 준다.
74. 압박이 세질수록 말투는 무너지는 게 아니라 원래의 말버릇, 문장 길이, 회피 방식이 더 선명해져야 한다.
75. 감각은 고르게 나누지 말고, 한 비트마다 주감각 하나를 먼저 세운 뒤 보조감각은 압력·오염·거리감처럼 한 기능만 맡겨라.
76. 감각 비유는 장면 안 원인, 물리적 변화, 인물 반응과 이어질 때만 쓴다. 원인 없이 떠 있는 장식 비유는 감각의 발생 원인, 물리적 변화, 인물 반응 순서로 다시 쓴다.
77. 짧은 문단은 결정타, 전환, 대사, 시간 압박 같은 효과 비트에만 아껴 쓴다. 인접한 행동-관찰-반응은 한 문단 안에서 처리한다.
78. 소품의 의미를 서술자가 바로 해설하지 않는다. 용도와 상징은 사용 방식, 실패, 손에 익은 정도, 상대 반응으로 드러낸다.
79. 선택 직전에는 생각보다 몸의 움직임이 먼저 보이게 쓴다. 내면 해석은 움직임 뒤에 짧게 붙인다.
80. 같은 콜백은 같은 문장으로 반복하지 말고 의미가 바뀐 각도로 되돌린다.

## 시점 공통 규칙

- 선택된 서사 시점이 1인칭이면 화자가 직접 보고 듣고 느끼고 추론한 것만 쓴다.
- 선택된 서사 시점이 3인칭 제한 시점이면 초점 인물이 알 수 있는 것만 쓴다.
- 다중 POV는 장면 단위로 명확히 전환하고, 한 장면 안에서는 하나의 시점 계약을 유지한다.
- 어떤 시점이든 타인의 속마음, 장면 밖 정보, 독자만 아는 결론을 서술자가 먼저 확정하지 않는다.

## 3인칭 제한 시점일 때만

- "그는/그가/그녀는/그녀가/자신이" 같은 3인칭 주어를 영어식으로 반복하지 않는다.
- 같은 인물을 가리킬 때는 이름, 직함, 무주어 행동문, 직접 행동문으로 주어를 분산한다.
- 한 문단 안에서 같은 인물 주어를 반복하지 않는다.
...
- 한 문장은 하나의 핵심 행동이나 이미지 단위를 중심에 두고, 겹치는 정보는 다음 문장으로 이어 준다.
...
- 묘사의 길이는 대상의 화려함이 아니라 장면에서 일으키는 변화량에 맞춘다. 변화가 없는 정보는 한 문장으로 통과시키고, 선택이나 대가가 생기는 지점은 행동·감각·반응을 붙여 호흡을 늦춘다.
...
- 독자가 이미 추론할 수 있는 의도, 성격, 관계 판단을 마지막에 장식적 판정문으로 덧붙이지 않는다. 구체 단서가 충분하면 그 문장은 빼고, 필요하면 손의 움직임, 상대 반응, 물건의 쓰임, 짧은 평문 결과로 남긴다.
- 시점이 흐려질 수 있는 방향 전환에서는 화자나 초점 인물을 짧게 다시 고정하고, 불필요한 주어 반복 대신 동작과 감각 반응으로 문장을 시작한다.
- 도입부 감각은 냄새, 촉각, 소리, 시각 중 장면의 압력을 가장 잘 드러내는 하나를 고르되, 첫 문장만으로도 독자가 누가 어디서 무엇을 인지하는지 잡을 수 있게 쓴다. 감각 단어를 나열하기보다 인물이 움직이며 닿고 보고 듣는 순서에 묻어 둔다.
- 신체 감각은 원인, 확인 행동, 다음 선택과 이어질 때 가장 자연스럽다. 장면의 첫 정보와 분리된 장식적 감각문으로 세우지 않는다.
- 소리와 분위기를 쓸 때는 실제로 닿은 물건, 떨린 표면, 멈춘 행동, 바뀐 호흡으로 감각을 장면 안에 고정한다. 감각 표현이 장면의 원인과 이어지지 않으면 장식이 된다.
- 웹소설식 짧은 문단은 클릭 압력과 시선 고정에 쓰되, 한 문장마다 빈 줄을 넣는 리듬을 기본값으로 만들지 않는다. 행동 하나, 관찰 하나, 그에 따른 반응 하나가 바로 이어지면 같은 문단에서 한 호흡으로 처리한다.
- 무기, 단말, 증표 같은 소품은 설명문으로 가치를 붙이지 말고 지금 장면에서 어떻게 쓰이고, 어디서 부족하고, 누구의 반응을 바꾸는지로 기능을 보인다.
- 압박 장면은 누군가의 시도, 상대의 저항, 그 결과 생긴 새 선택을 짧은 비트로 이어 간다.
- 대사와 지문은 더 짧고 자연스러운 쪽을 고른다. 설정 대사는 실제 상황의 목표, 오해, 위협, 거래 안에서 흘린다. 지문은 복잡한 배경이나 규칙을 짧게 정리할 때 쓴다.
- 추상어 세 개를 독립 문단으로 세운 뒤 그 세 가지가 맞물린다고 정리하는 리듬은 장면을 보고서처럼 만든다. 필요하면 세 요소를 각각 행동, 반응, 결과 안에 묻어 둔다.
- 좋은 비트는 구체적 행동, 소리, 즉각적인 짧은 생각으로 닫는다. "쿵." 다음 "아프겠다."처럼 독자가 바로 따라가는 반응을 우선한다.

## 문장 운용 원칙

- 장면에서 확인된 사실은 직진 문장으로 선명하게 전달한다.
- 그리고, 그러나, 그런데, 하지만 같은 접속사가 이어지면 앞뒤 인과를 정리하고 필요 없는 접속사는 뺀다.
- 피동과 명사화가 문장을 흐리면 능동 동사로 바꾼다.
...
- 감정이 바뀌는 순간은 손이 멈추는지, 말끝이 흐려지는지, 시선을 피하는지처럼 관찰 가능한 변화로 먼저 보여 준다.
- 다인 장면에서는 정보 전달보다 압박, 협상, 회피, 떠보기 같은 직접 공방을 우선한다.
- 대사 주변에는 손짓, 말끝, 침묵, 현장 디테일을 배치해 장면의 입체를 살린다.
- 설명은 시간 압축과 연결용으로 짧게 쓰고, 회차의 핵심 갈등은 장면 안에서 전개한다.
- 서술 시간은 장면 중요도에 맞춰 배분한다. 독자가 체감해야 할 변화는 가까이 붙어 보여 주고, 결과만 알면 되는 절차와 이동은 과감히 건너뛴다.
- 돌발성과 강조는 표식어보다 행동과 감각으로 전달한다.
- 부사로 속도와 감정을 덧칠하지 말고, 동사와 명사 선택으로 문장의 힘을 만든다.
...
- 회고형 선언이나 판세 비유로 의미를 요약하지 말고, 마지막 행동이 만든 새 압력을 보여 준다.`;
}
...
- 이번 장면에서 확대해 보여 줄 대상 1-2개와 간결하게 넘길 정보를 먼저 구분한다. 확대 기준은 장면의 선택, 대가, 폭로, 관계 변화, 위협 신호, 회수 여부다.
...
Planner/Composer already handled chapter intent, context selection, rule stack, and risk scanning. Writer only turns those inputs into readable prose. Do not output plans, checklists, review notes, or state settlement.

=== CHAPTER_TITLE ===
(Chapter title. Do not include "Chapter X". Do not repeat or closely echo existing titles. If recent title history or high-frequency title words are provided, avoid repeated roots and imagery.)

=== CHAPTER_CONTENT ===
(Chapter prose. Target ${lengthSpec.target} words, recommended range ${lengthSpec.softMin}-${lengthSpec.softMax} words)

[Important] Output only CHAPTER_TITLE and CHAPTER_CONTENT.
Tracking files such as checklists, state cards, hook pools, and summaries are handled by Planner/Composer, Observer/Settler, and Auditor/Reviser.`;
  }

  if (language === "ko") {
    return `## 출력 형식 (엄수)

Planner/Composer는 이미 본장 의도, 컨텍스트 선택, 규칙 스택, 위험 스캔을 처리했다. Writer는 이 입력을 읽히는 본문으로 구현하고 계획, 검사표, 심사 의견, 상태 정산은 출력하지 않는다.

=== CHAPTER_TITLE ===
(장 제목. "제X화"를 붙이지 않는다. 기존 제목과 같거나 비슷한 제목을 반복하지 않는다. 최근 제목 기록이나 고빈도 제목어가 있으면 반복 어근과 이미지를 피한다.)

=== CHAPTER_CONTENT ===
(본문. 목표 ${lengthSpec.target}자, 권장 구간 ${lengthSpec.softMin}-${lengthSpec.softMax}자)

[중요] 이번 출력은 CHAPTER_TITLE, CHAPTER_CONTENT 두 구역만 포함한다.
검사표, 상태 카드, 복선 풀, 요약 같은 추적 파일은 Planner/Composer, Observer/Settler, Auditor/Reviser가 처리하므로 출력하지 않는다.`;
  }
...
| 점검 항목 | 이번 화 기록 | 비고 |
|--------|----------|------|
| 아웃라인 고정점 | 현재 권/단계 + 이번 화가 밀어야 할 구체 노드 | 노드를 건너뛰거나 후속 사건을 앞당겨 소모하지 않는다 |
| 컨텍스트 범위 | 제X화-제Y화 / 상태 카드 / 설정 파일 | |
| 현재 앵커 | 장소 / 상대 / 보상 목표 | 앵커는 구체적이어야 한다 |
${resourceRow}| 회수 대기 복선 | 실제 hook_id 기입(없으면 none) | 복선 풀과 일치 |
| 이번 화 갈등 | 한 문장 요약 | |
| 회차 유형 | ${gp.chapterTypes.join("/")} | |
| 위험 스캔 | OOC/정보 월권/설정 충돌${gp.powerScaling ? "/전투력 붕괴" : ""}/리듬/어휘 피로 | |`;
...
| 정산 항목 | 이번 화 기록 | 비고 |
|--------|----------|------|
| 자원 장부 | 기초 X / 증감 +Y / 기말 Z | 증감이 없으면 +0 |
...
=== UPDATED_STATE ===
(업데이트된 전체 상태 카드, Markdown 표 형식)
${updatedLedger}
=== UPDATED_HOOKS ===
(업데이트된 전체 복선 풀, Markdown 표 형식)

=== CHAPTER_SUMMARY ===
(이번 화 요약, Markdown 표 형식, 반드시 아래 열 포함)
| 회차 | 제목 | 등장인물 | 핵심 사건 | 상태 변화 | 복선 동향 | 감정 기조 | 회차 유형 |
|------|------|----------|----------|----------|----------|----------|----------|
| N | 이번 화 제목 | 인물1,인물2 | 한 문장 요약 | 핵심 변화 | H01 심음/H02 진행 | 감정 흐름 | ${gp.chapterTypes.length > 0 ? gp.chapterTypes.join("/") : "전환/갈등/절정/수습"} |

=== UPDATED_SUBPLOTS ===
(업데이트된 전체 서브플롯 진행판, Markdown 표 형식)
| 서브플롯ID | 서브플롯명 | 관련 인물 | 시작 화 | 최근 활성 화 | 경과 화수 | 상태 | 진행 개요 | 회수 ETA |
|--------|--------|----------|--------|------------|----------|------|----------|---------|

=== UPDATED_EMOTIONAL_ARCS ===
(업데이트된 전체 감정선, Markdown 표 형식)
| 인물 | 회차 | 감정 상태 | 촉발 사건 | 강도(1-10) | 흐름 방향 |
|------|------|----------|----------|------------|----------|

=== UPDATED_CHARACTER_MATRIX ===
(업데이트된 인물 상호작용 매트릭스, 세 개 하위 표)

### 인물 프로필
| 인물 | 핵심 태그 | 반전 디테일 | 말투 | 성격 바탕 | 주인공과의 관계 | 핵심 동기 | 현재 목표 |
|------|----------|----------|----------|----------|------------|----------|----------|

### 만남 기록
| 인물A | 인물B | 첫 만남 화 | 최근 상호작용 화 | 관계 성격 | 관계 변화 |
|-------|-------|------------|------------|----------|----------|

### 정보 경계
| 인물 | 아는 정보 | 모르는 정보 | 정보 출처 화 |
|------|----------|----------|------------|`;
  }
