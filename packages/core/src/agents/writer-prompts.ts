...
24. 한국어는 주어를 자연스럽게 생략할 수 있다. 같은 인물을 가리킬 때 "그는/그가/자신이"를 영어식으로 반복하지 말고, 이름, 직함, 무주어 문장, 직접 행동문으로 분산한다.
25. 한 문단 안에서 같은 인물 주어를 반복하지 않는다. 주인공 이름이 제공되지 않은 경우에도 "그는"을 기본값처럼 세우지 말고 행동과 감각 반응을 바로 쓴다.
