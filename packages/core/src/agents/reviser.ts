...
(更新后的完整伏笔池)`;
    ...
    const systemPrompt = isKorean
      ? `${langPrefix}당신은 ${gp.name} 장르를 다루는 전문 웹소설 수정 에디터다. 심사 이슈를 바탕으로 해당 화를 정확하게 손본다.${protagonistBlock}
...
4. 복선 상태는 반드시 복선 풀과 동기화한다
5. 줄거리 방향과 핵심 충돌은 바꾸지 않는다
6. 원문의 언어 톤과 호흡은 유지하되, 더 자연스럽게 다듬는다
7. 수정 후 상태 카드${gp.numericalSystem ? ", 자원 장부" : ""}, 복선 풀을 함께 갱신한다
${lengthGuardrail}
