...
function buildDimensionNote(
  ...
  if (id === 10 && words.length > 0) {
    return language === "en"
      ...
      : language === "ko"
        ? `피로도 단어: ${words.join(", ")}. 반복 표현, 번역투 표식어, 갑작스러운 감탄/동작 부사를 함께 확인해 3000자당 1회 초과 시 경고하세요.`
  ...
  switch (id) {
    case 8:
      return language === "en"
        ? "Audit as a diagnostic reader, not a rewriter: identify where the prose becomes hard to picture or parse on first read. Check whether key emotional, relational, and payoff beats are dramatized in scene instead of only being reported after the fact. Flag chapters where the narrator explains motives, stakes, or meaning that the scene already makes inferable. Also flag decorative narrator verdicts that follow concrete evidence and restate inferred motive or character judgment in metaphor instead of letting the scene carry it. Also flag paragraphs that jump to isolated micro-detail or abstract verdict before the reader can picture the physical setup."
        : language === "ko"
          ? "리뷰어 시점으로 읽고, 독자가 한 번에 그림을 잡는지 확인하세요. 핵심 감정 변화, 관계 변화, 카타르시스 비트가 장면 안에서 드러나는지 보고, 행동·표정·대사·감각의 증거만으로도 감정과 의도가 읽히는지 살피세요. 공간과 형상이 먼저 세워지고 그 위에 세부 디테일과 판단이 놓이는지, 묘사가 핵심 대상에 집중되며 강약을 가지는지, 서술 시간이 실제 사건 시간보다 장면 중요도에 맞게 배분되는지, 그리고 평문 연결 문장이 읽기 속도를 안정적으로 받치는지도 함께 점검하세요. 그는/그녀는 같은 3인칭 대명사 주어가 영어 번역투처럼 반복되는지, 감각 자체가 주어가 되는 도입문이 반복되는지, 비문처럼 걸리는 인식 문장이 감각 증거 없이 결론만 말하는지도 표시하세요. 구체 사물·행동 묘사 뒤에 서술자가 인물의 의도나 성격을 장식적 비유/판정문으로 닫아 독자의 추론을 대신하는지도 확인하세요. 시점 인물이 알 수 없는 정보나 타인의 속마음을 서술자가 먼저 해설하는지, 처음 등장하는 고유명사가 관계, 기능, 위협 없이 이름만 던져지는지, 장면의 갈등이 인물의 욕망과 방해로 작동하는지, 대사와 지문 중 더 짧고 자연스러운 전달 방식을 선택했는지도 점검하세요. 회차 끝이 장면의 결과 대신 회고형 선언이나 판세 비유로 닫히는지 확인하고, 마지막 줄이 실제 선택, 되돌릴 수 없는 결과, 상대 반응을 남기는지 보세요. 품질 원인을 추정하는 포괄 라벨만 적지 말고 관찰 가능한 반복 패턴, 문장 위치, 독서 흐름이 흔들리는 이유를 근거로 적으세요. 문제 위치, 독서 흐름이 흔들리는 이유, 최소 수정 방향을 분리해 적으세요."
          : "用诊断型读者视角审阅，指出哪些地方读者无法一遍看清、需要回读。检查关键情绪、关系变化和回收段落是否只是事后摘要；如果场景已经足够明显，就不要再让叙述者重复解释动机、风险或意义。同时标记具体物件或行动描写之后，叙述者又用装饰性比喻/判断句替读者盖棺定论人物动机或性格的段落。同时标记那些在读者还没看清空间与轮廓前，就先跳到孤立细节或抽象判断的段落。";
    case 19:
      return language === "en"
    ...
    case 17:
      return language === "en"
        ...
        : language === "ko"
          ? "중요한 감정 변화, 관계 이동, 카타르시스 비트가 장면으로 충분히 펼쳐지는지 점검하세요. 인물의 멈춤, 손짓, 시선, 접근 같은 행동도 그 행동을 부른 자극과 함께 읽히는지 확인하세요. 장면마다 강조해야 할 디테일과 빠르게 넘어갈 정보가 구분되어 읽기 속도가 유지되는지, 문단마다 핵심 디테일이 선명하게 남는지, 선택·대가·폭로·관계 변화·위협 신호는 충분히 확대되고 이동·반복 절차·이미 이해된 정보는 압축되는지도 함께 보세요. 설명 문장이 전환과 압축의 역할을 분명하게 맡는지도 함께 보세요."
...
검토 우선순위:
- 구조 -> 인물/갈등 -> 장면 -> 문장 순서로 우선순위를 정하세요.
- 각 이슈에는 독자 영향과 권장 수정 폭을 함께 적으세요.
- 회차가 작은 보상 1개와 다음 질문 1개를 모두 남기는지 확인하세요.
- 주인공의 능동적 선택과 그 선택의 대가가 장면 안에서 보이는지 확인하세요.
- 장면마다 욕망 / 행동 / 변화가 실제로 일어나는지 확인하세요.
- 문장 문제처럼 보이는 지점도 장면 목표, 방해, 전환이 비어 생긴 문제인지 먼저 구분하세요.
- 본문 근거가 부족하거나 독자 체감 저해가 불명확하면 info로 낮추세요.
- 반복 패턴, 실제 막힘, 명백한 충돌이 있을 때만 이슈로 올리세요.
- 각 이슈의 suggestion에는 보존할 장점이나 손대지 말아야 할 장면 장치를 함께 적으세요.
...
- 심각도는 독자 체감 손상, 연속성 파급, 되돌림 난이도를 기준으로 나눈다.
...
- 모순은 사실 충돌, 근거 없는 추정, 시간 역행, 훅 상태 불일치, 소급 편집으로 분류하고, FAIL은 사실 충돌에만 사용한다.
